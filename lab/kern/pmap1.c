...
page_init(void)
{
	...
	uint32_t page_start = 0, page_end = PGSIZE-1;
	uint32_t kernel_start = EXTPHYSMEM;
	//uint32_t kernel_start = 0x10000;
	uint32_t kernel_end = (uint32_t)PADDR(nextfree);
        //extern char end[];
	//uint32_t kernel_end = (uint32_t)ROUNDUP(PADDR(end), PGSIZE);
	cprintf("Reached....end = %0x\n", kernel_end);
	int i;
	for (i = 0; i < npages; i++) {
		pages[i].pp_ref = 0;

	  	if(i == 0) {}
	  	else if( (page_start >= IOPHYSMEM && page_start < EXTPHYSMEM) ||
		   (page_end >= IOPHYSMEM && page_end < EXTPHYSMEM)) {}
	  	else if( (page_start >= kernel_start && page_start < kernel_end) ||
		   (page_end >= kernel_start && page_end < kernel_end)) {}
               else if( (page_start >= PADDR(BOOT_PAGE_TABLE_START) && page_start < PADDR(BOOT_PAGE_TABLE_END)) ||
                   (page_end >=PADDR(BOOT_PAGE_TABLE_START) && page_end < PADDR(BOOT_PAGE_TABLE_END))) {}
                else if( page_start == MPENTRY_PADDR) {}
	  	else {
			pages[i].pp_link = page_free_list;
			page_free_list = &pages[i];
		}
	...
	//	if (page2pa(p) < 0x200000) {
			//cprintf("Reached....aadded page %0x\n", page2pa(p));
	//	}
	}
...
page_free(struct Page *pp)
{
	...
	for (p=page_free_list;p!=0;p=p->pp_link) {
		if (p == pp) {
			return;
		}
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (va_i=(void *)ROUNDDOWN(va, PGSIZE); va_i<ROUNDUP((va+len), PGSIZE); va_i+=PGSIZE) {

		pte_t *pte = pml4e_walk(env->env_pml4e, va_i, 1);

		if ((*pte & (perm|PTE_P)) != (perm|PTE_P)) {
			uintptr_t invalid_addr = (uintptr_t)KADDR(PTE_ADDR(*pte));
			if (invalid_addr < (uintptr_t)va) {
				user_mem_check_addr = (uintptr_t)va;
			} else if (invalid_addr >= (uintptr_t)(va+len)) {
				user_mem_check_addr = (uintptr_t)(va+len-1);
			} else {
				user_mem_check_addr = invalid_addr;
			}
...
check_page_free_list(bool only_low_memory)
{
	...
	cprintf("Reached....check_page_free_list() succeeded\n");
}
