...
struct Page *page_free_list;	// Free list of physical pages
...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		...
		if(i == 0) {}
				...
				(page_end >= IOPHYSMEM && page_end < EXTPHYSMEM)) {count1++;}
		else if( (page_start >= kernel_start && page_start < kernel_end) ||
				(page_end >= kernel_start && page_end < kernel_end)) {count2++;}
		else if( (page_start >= PADDR(BOOT_PAGE_TABLE_START) && page_start < PADDR(BOOT_PAGE_TABLE_END)) ||
				(page_end >=PADDR(BOOT_PAGE_TABLE_START) && page_end < PADDR(BOOT_PAGE_TABLE_END))) {count3++;}
