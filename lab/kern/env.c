...
envid2env(envid_t envid, struct Env **env_store, bool checkperm)
{
	...
	if (e->env_status == ENV_FREE || e->env_id != envid) {
		*env_store = 0;
		cprintf("\nReached env id not matching: e->env_id:%d envid:%d", e->env_id, envid);
		cprintf("\nenv_status:%d", e->env_status);
		return -E_BAD_ENV;
	}
	...
	if (checkperm && e != curenv && e->env_parent_id != curenv->env_id) {
		*env_store = 0;
		cprintf("\nReached parent id not equal to env id");
		return -E_BAD_ENV;
	}
...
env_setup_vm(struct Env *e)
{
	...
	struct Page *p2 = NULL, *p3 = NULL;

	if (!(p2= page_alloc(ALLOC_ZERO))||!(p3 = page_alloc(ALLOC_ZERO))) {
		return -E_NO_MEM;
	}

	e->env_pml4e = KADDR(PTE_ADDR(page2pa(p)));
	//cprintf("\ne->env_pml4e:", e->env_pml4e);
	e->env_pml4e[PML4(UTOP)] = page2pa(p2) | PTE_U | PTE_W | PTE_P;
	pdpe_t *pdpe = KADDR(PTE_ADDR(e->env_pml4e[PML4(UTOP)]));
	pdpe[PDPE(UTOP)] = page2pa(p3) | PTE_U | PTE_W | PTE_P;

	pte_t *pgdir = KADDR(PTE_ADDR(pdpe[PDPE(UTOP)]));
	
	p->pp_ref++;
	p2->pp_ref++;
	p3->pp_ref++;

	pdpe_t *kernel_pdpe = KADDR(PTE_ADDR(boot_pml4e[PML4(UTOP)]));
	pte_t *kernel_pgdir = KADDR(PTE_ADDR(kernel_pdpe[PDPE(UTOP)]));

	// aj-comment pgdir entries should be copied from PTA(UTOP), not from 0
	for (i=PTX(UTOP); i<NPDENTRIES; i++) {
	...
	/*
	// understanding.....
	void *va1 = (void *)0xabcde000;
	struct Page *p1 = page_alloc(ALLOC_ZERO);
	page_insert(e->env_pml4e, p1, (void *)0xabcde000, PTE_U|PTE_P|PTE_W);
	//pte_t* ra = pml4e_walk(e->env_pml4e, (void *)0xabcde000, 0);


	pte_t *pdpe_pa = (pte_t *)(*(e->env_pml4e + PML4(va1)));

	pte_t *pdpe_va = (pte_t *)(KADDR((uint64_t)pdpe_pa));
	pdpe_va += PDPE(va1);
	pte_t *pgdir_pa = (pte_t *)(*pdpe_va);
	*/
...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
	...
	e->env_tf.tf_rsp = USTACKTOP;// - PGSIZE;
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (va_i=(uintptr_t)ROUNDDOWN(va, PGSIZE); va_i<ROUNDUP((uintptr_t)va+len, PGSIZE); va_i+=PGSIZE) {
		struct Page *p = NULL;
		if (!(p=page_alloc(ALLOC_ZERO))) {
			panic("memory insufficient");
		}

		page_insert(e->env_pml4e, p, (void*)va_i, PTE_U|PTE_W|PTE_P);
	}
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	page_insert((pml4e_t *)e->env_pml4e, page_alloc(0), (void*)(USTACKTOP-PGSIZE), PTE_U|PTE_W);
}
