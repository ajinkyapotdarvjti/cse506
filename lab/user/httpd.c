...
send_data(struct http_request *req, int fd)
{
	...
	ssize_t n = -1;
	if((n=read(fd, file_data, file_size)) != file_size) {
		cprintf("Failed to read the file...\n");
		return -1;
	}
	cprintf("file read, file_data = %s\n", file_data);

        r = snprintf(buf, 512, "%s\r\n",
                               file_data);

	cprintf("response created\n");

        if (write(req->sock, buf, r) != r) {
		cprintf("failed to write on socket...\n");
                return -1;
	}

	cprintf("send_data() successful...\n");
        return 0;
